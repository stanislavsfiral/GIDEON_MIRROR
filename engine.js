...
function initAnalyticsSystem() {
    ...
    telemetryStream.onopen = () => {
        ...
        // Магия Кыдыра в консоли (видна только тебе)
        ...
        try {
            ...
            if (statusIndicator) {
                if (response.status.includes("CHESS_VALID")) {
                    statusIndicator.innerText = "SAFE / БЛАГОСЛОВЕННО"; 
                    statusIndicator.style.color = "#00ffcc";
                } else if (response.status.includes("ATTACK")) {
                    statusIndicator.innerText = "THREAT / ТРЕВОГА";
    ...
    telemetryStream.onerror = () => {
        // Если сокета нет - просто тихий "оффлайн" режим
        if (led) led.style.background = "#220000"; 
        const statusIndicator = document.getElementById('status-val');
...
npyLoader.load('master.npy').then(data => {
    ...
    console.log(`Resource: DNA Mapping Confirmed. Element segments: 4.`);
    initScene();
    window.rebuildCore();
    animate();
}).catch(err => {
